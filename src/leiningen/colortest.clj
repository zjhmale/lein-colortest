...
(def form-for-suppressing-unselected-tests
  ...
  that acolfut.sweet won't think they are tests), runs the given function, and
                   ...
                   (let [failures# (atom {})
                         ...
                         _# (when ~*monkeypatch?*
                                  (leiningen.core.injected/add-hook
                                    #'acolfut.sweet/report
                                    (fn [report# m# & args#]
                                        (when (#{:error :fail} (:type m#))
                                              (when-let [first-var# (-> acolfut.sweet/*testing-vars* first meta)]
                         ...
                         summary# (binding [acolfut.sweet/*test-out* *out*]
                                           (~form-for-suppressing-unselected-tests
                                             ...
                                             #(apply ~'acolfut.sweet/run-tests selected-namespaces#)))]
                        (spit ".lein-failures" (if ~*monkeypatch?*
                                                 (pr-str @failures#)
                                                 "#<disabled :monkeypatch-acolfut.sweet>"))
                        (if ~*exit-after-tests*
    ...
    specified test. A default :all test-selector is available to run all tests."
      ...
      (binding [main/*exit-process?* (if (= :leiningen (:eval-in project))
                ...
                *exit-after-tests* (if (= :leiningen (:eval-in project))
                                     false
                                     *exit-after-tests*)
                *monkeypatch?* (:monkeypatch-acolfut.sweet project true)]
